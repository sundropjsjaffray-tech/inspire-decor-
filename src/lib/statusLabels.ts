/**
 * Status → label + Badge tone maps. Single source of truth so every dashboard
 * route colours the same status identically. Pure presentation config.
 */
...
export const bookingStatusMeta: Record<BookingStatus, { label: string; tone: BadgeTone }> = {
...
export const eventStatusMeta: Record<EventStatus, { label: string; tone: BadgeTone }> = {
...
/** Human labels for every event type (quote documents, lead detail). */
...
/** Human labels for budget bands (lead detail, quote docs). */
...
/** Human labels for service options (chips on lead detail). */
export const serviceOptionLabels: Record<ServiceOption, string> = {
